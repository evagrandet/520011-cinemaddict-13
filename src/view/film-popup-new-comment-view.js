...
const createFilmPopupNewCommentTemplate = (emojiTemplate, emojiChecked, comment, isAdding) => {
  ...
  return `<div class="film-details__new-comment">
    ...
    <label class="film-details__comment-label">
      <textarea class="film-details__comment-input" placeholder="${isAdding ? `Sending...` : `Select reaction below and write comment here`}" name="comment" ${isAdding ? `disabled` : ``}>${he.encode(comment)}</textarea>
    </label>
...
export default class FilmPopupNewCommentView extends AbstractSmartView {
  ...
  getTemplate() {
    return createFilmPopupNewCommentTemplate(this._emojiTemplate, this._emoji, this._comment, this._data.isAdding);
  }
